    ...
    def vikingAttack(self):
        if len(self.vikingArmy) < 1 or len(self.saxonArmy) < 1:
            ...
            if random.randint(0,5)==3:
                print(attacking_viking.battleCry())
            result = attacked_saxon.receiveDamage(attacking_viking.strength)
            if attacked_saxon.health <= 0:
                self.saxonArmy.remove(attacked_saxon) 
            return result
  
    def saxonAttack(self):
        if len(self.saxonArmy) < 1 or len(self.saxonArmy) < 1:
            self.showStatus()
        else:
            attacking_saxon = random.choice(self.saxonArmy)
            attacked_viking = random.choice(self.vikingArmy)
            result = attacked_viking.receiveDamage(attacking_saxon.strength)
